...
- `/cases` All Cases list from `public.cases` (empty lists show one line, not an empty table). Grouped by Referred Firm. Toolbar search: case number or client name (resolved display name). Zero search hits say `No cases match {query}` plus Clear — not the empty-pipeline line. Filters: firm, status, next step, assigned. Missing routes and missing cases keep staff chrome and a Back to All Cases link. List and case loading use a skeleton.
